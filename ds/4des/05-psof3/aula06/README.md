# Aula06 - Avaliação Prática - Techman
...
administrador e o registro de um novo equipamento no sistema.<br>Você precisará entregar para o instrutor um documento em formato *.png contendo atividades, transições e ações que representam esse cenário.
...
Ao final da sua atividade você precisa entregar ao instrutor em um repositório do github os seguintes arquivos:

- Um arquivo em formato *.png contendo o **MER-DER Conceitual** (Diagrama Entidade Relacionamento) criado por você;
- Um arquivo em formato *.png contendo o **UML-DA** (Diagrama de Atividades) com atividades, transições e ações.
- Os códigos fonte da aplicação **web** criada contendo a tela de login, recurso para listagem, cadastro e exclusão de equipamentos, e listagem e cadastro de comentários para determinado equipamento.
- Os códigos do back-end **api**, criação e importação (semeadura [`seed.js` ou `*.sql`]) dos dados para o **banco de dados relacional** utilizando o ORM **prisma** com Banco de Dados **MySQL** que aprendemos em aula.
...
## [Repositório com a solução fullstack proposta](https://github.com/wellifabio/techman-fullstack-2025.git)